# hover-assessment
git push -u origin feature/hover-figma-section

to login :shopify login 

email rayanalhassan2000@gmail.com
password of store rayan@hover

to enter cli:
shopify theme dev --store test-hover-store.myshopify.com 
then e to enter admin panel
link to test
https://test-hover-store.myshopify.com/?preview_theme_id=156866511085



after editing daft 
shopify theme push

from 4-7pm 26/1 : 
setup:
1-create a shopify account 
2-download theme on local
3-open shpify code on vs code 
4-install shopify cli 
5-sync vs code with admin panel shopify 

start editing with vs code 
create for section 1 : sections/custom-product-details.liquid
assets/custom-product-details.css

finish the section one and  its responsivens as carousel 